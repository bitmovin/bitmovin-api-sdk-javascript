...
export default class ClearkeyApi extends BaseAPI {
  ...
  /**
   * @summary Add ClearKey DRM to CMAF
  ...
  public create(encodingId: string, muxingId: string, clearKeyDrm?: ClearKeyDrm): Promise<ClearKeyDrm> {
    ...
    return this.restClient.post<ClearKeyDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/clearkey', pathParamMap, clearKeyDrm).then((response) => {
      return new ClearKeyDrm(response);
    });
  }

  /**
   * @summary Delete ClearKey DRM from CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/clearkey/{drm_id}', pathParamMap).then((response) => {
      return new BitmovinResponse(response);
    });
  }

  /**
   * @summary ClearKey DRM Details of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
   * @param {string} drmId Id of the ClearKey DRM configuration.
   * @throws {RequiredError}
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<ClearKeyDrm> {
    ...
    return this.restClient.get<ClearKeyDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/clearkey/{drm_id}', pathParamMap).then((response) => {
      return new ClearKeyDrm(response);
    });
  }

  /**
   * @summary List ClearKey DRMs of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public list(encodingId: string, muxingId: string, queryParams?: ClearKeyDrmListQueryParams): Promise<PaginationResponse<ClearKeyDrm>> {
    ...
    return this.restClient.get<PaginationResponse<ClearKeyDrm>>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/clearkey', pathParamMap, queryParams).then((response) => {
