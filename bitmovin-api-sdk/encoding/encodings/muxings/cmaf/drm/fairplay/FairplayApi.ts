...
export default class FairplayApi extends BaseAPI {
  ...
  /**
   * @summary Add FairPlay DRM to CMAF
  ...
  public create(encodingId: string, muxingId: string, fairPlayDrm?: FairPlayDrm): Promise<FairPlayDrm> {
    ...
    return this.restClient.post<FairPlayDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/fairplay', pathParamMap, fairPlayDrm).then((response) => {
      return new FairPlayDrm(response);
    });
  }

  /**
   * @summary Delete FairPlay DRM from CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/fairplay/{drm_id}', pathParamMap).then((response) => {
      return new BitmovinResponse(response);
    });
  }

  /**
   * @summary FairPlay DRM Details of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
   * @param {string} drmId Id of the FairPlay DRM configuration.
   * @throws {RequiredError}
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<FairPlayDrm> {
    ...
    return this.restClient.get<FairPlayDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/fairplay/{drm_id}', pathParamMap).then((response) => {
      return new FairPlayDrm(response);
    });
  }

  /**
   * @summary List FairPlay DRMs of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public list(encodingId: string, muxingId: string, queryParams?: FairPlayDrmListQueryParams): Promise<PaginationResponse<FairPlayDrm>> {
    ...
    return this.restClient.get<PaginationResponse<FairPlayDrm>>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/fairplay', pathParamMap, queryParams).then((response) => {
