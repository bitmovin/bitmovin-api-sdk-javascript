...
export default class PlayreadyApi extends BaseAPI {
  ...
  /**
   * @summary Add PlayReady DRM to CMAF
  ...
  public create(encodingId: string, muxingId: string, playReadyDrm?: PlayReadyDrm): Promise<PlayReadyDrm> {
    ...
    return this.restClient.post<PlayReadyDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/playready', pathParamMap, playReadyDrm).then((response) => {
      return new PlayReadyDrm(response);
    });
  }

  /**
   * @summary Delete PlayReady DRM from CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/playready/{drm_id}', pathParamMap).then((response) => {
      return new BitmovinResponse(response);
    });
  }

  /**
   * @summary PlayReady DRM Details of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
   * @param {string} drmId Id of the PlayReady DRM configuration.
   * @throws {RequiredError}
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<PlayReadyDrm> {
    ...
    return this.restClient.get<PlayReadyDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/playready/{drm_id}', pathParamMap).then((response) => {
      return new PlayReadyDrm(response);
    });
  }

  /**
   * @summary List PlayReady DRMs of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public list(encodingId: string, muxingId: string, queryParams?: PlayReadyDrmListQueryParams): Promise<PaginationResponse<PlayReadyDrm>> {
    ...
    return this.restClient.get<PaginationResponse<PlayReadyDrm>>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/playready', pathParamMap, queryParams).then((response) => {
