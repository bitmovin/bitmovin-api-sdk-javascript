...
export default class CencApi extends BaseAPI {
  ...
  public create(encodingId: string, muxingId: string, cencDrm?: CencDrm): Promise<CencDrm> {
    ...
    return this.restClient.post<CencDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/cenc', pathParamMap, cencDrm).then((response) => {
      return new CencDrm(response);
    });
  }

  /**
   * @summary Delete CENC DRM from CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/cenc/{drm_id}', pathParamMap).then((response) => {
      return new BitmovinResponse(response);
    });
  }

  /**
   * @summary CENC DRM Details of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
   * @param {string} drmId Id of the cenc drm.
   * @throws {RequiredError}
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<CencDrm> {
    ...
    return this.restClient.get<CencDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/cenc/{drm_id}', pathParamMap).then((response) => {
      return new CencDrm(response);
    });
  }

  /**
   * @summary List CENC DRMs of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public list(encodingId: string, muxingId: string, queryParams?: CencDrmListQueryParams): Promise<PaginationResponse<CencDrm>> {
    ...
    return this.restClient.get<PaginationResponse<CencDrm>>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/cenc', pathParamMap, queryParams).then((response) => {
