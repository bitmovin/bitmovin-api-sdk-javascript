...
export default class MarlinApi extends BaseAPI {
  ...
  /**
   * @summary Add Marlin DRM to CMAF
  ...
  public create(encodingId: string, muxingId: string, marlinDrm?: MarlinDrm): Promise<MarlinDrm> {
    ...
    return this.restClient.post<MarlinDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/marlin', pathParamMap, marlinDrm).then((response) => {
      return new MarlinDrm(response);
    });
  }

  /**
   * @summary Delete Marlin DRM from CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/marlin/{drm_id}', pathParamMap).then((response) => {
      return new BitmovinResponse(response);
    });
  }

  /**
   * @summary Marlin DRM Details of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
   * @param {string} drmId Id of the Marlin DRM configuration.
   * @throws {RequiredError}
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<MarlinDrm> {
    ...
    return this.restClient.get<MarlinDrm>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/marlin/{drm_id}', pathParamMap).then((response) => {
      return new MarlinDrm(response);
    });
  }

  /**
   * @summary List Marlin DRMs of CMAF
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the CMAF muxing
  ...
  public list(encodingId: string, muxingId: string, queryParams?: MarlinDrmListQueryParams): Promise<PaginationResponse<MarlinDrm>> {
    ...
    return this.restClient.get<PaginationResponse<MarlinDrm>>('/encoding/encodings/{encoding_id}/muxings/cmaf/{muxing_id}/drm/marlin', pathParamMap, queryParams).then((response) => {
