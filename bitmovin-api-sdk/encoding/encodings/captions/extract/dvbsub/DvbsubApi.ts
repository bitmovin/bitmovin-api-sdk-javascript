...
export default class DvbsubApi extends BaseAPI {
  ...
  public create(encodingId: string, dvbSubtitleSidecarDetails?: DvbSubtitleSidecarDetails): Promise<DvbSubtitleSidecarDetails> {
    ...
    return this.restClient.post<DvbSubtitleSidecarDetails>('/encoding/encodings/{encoding_id}/captions/extract/dvbsub', pathParamMap, dvbSubtitleSidecarDetails).then((response) => {
      return new DvbSubtitleSidecarDetails(response);
    });
  }

  /**
   * @summary Delete Extract DVB-SUB Subtitle
   * @param {string} encodingId Id of the encoding.
  ...
  public delete(encodingId: string, subtitleId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/captions/extract/dvbsub/{subtitle_id}', pathParamMap).then((response) => {
      return new BitmovinResponse(response);
    });
  }

  /**
   * @summary Get Extract Subtitle DVB-SUB Details
   * @param {string} encodingId Id of the encoding.
   * @param {string} subtitleId Id of the subtitle.
   * @throws {RequiredError}
   * @memberof DvbsubApi
  ...
  public get(encodingId: string, subtitleId: string): Promise<DvbSubtitleSidecarDetails> {
    ...
    return this.restClient.get<DvbSubtitleSidecarDetails>('/encoding/encodings/{encoding_id}/captions/extract/dvbsub/{subtitle_id}', pathParamMap).then((response) => {
      return new DvbSubtitleSidecarDetails(response);
    });
  }

  /**
   * @summary List Extract DVB-SUB Subtitle
   * @param {string} encodingId Id of the encoding.
  ...
  public list(encodingId: string, queryParams?: DvbSubtitleSidecarDetailsListQueryParams): Promise<PaginationResponse<DvbSubtitleSidecarDetails>> {
    ...
    return this.restClient.get<PaginationResponse<DvbSubtitleSidecarDetails>>('/encoding/encodings/{encoding_id}/captions/extract/dvbsub', pathParamMap, queryParams).then((response) => {
