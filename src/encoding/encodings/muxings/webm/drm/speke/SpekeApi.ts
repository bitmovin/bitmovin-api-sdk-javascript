...
export default class SpekeApi extends BaseAPI {
  ...
  /**
   * @summary Add SPEKE DRM key provider to WebM
  ...
  public create(encodingId: string, muxingId: string, spekeDrm?: SpekeDrm): Promise<SpekeDrm> {
    ...
    return this.restClient.post<SpekeDrm>('/encoding/encodings/{encoding_id}/muxings/webm/{muxing_id}/drm/speke', pathParamMap, spekeDrm).then((response) => {
      return map(response, SpekeDrm);
    });
  }

  /**
   * @summary Delete SPEKE DRM from WebM
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the WebM muxing
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/webm/{muxing_id}/drm/speke/{drm_id}', pathParamMap).then((response) => {
      return map(response, BitmovinResponse);
    });
  }

  /**
   * @summary SPEKE DRM Details of WebM
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the WebM muxing
   * @param {string} drmId Id of the SPEKE DRM.
   * @throws {BitmovinError}
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<SpekeDrm> {
    ...
    return this.restClient.get<SpekeDrm>('/encoding/encodings/{encoding_id}/muxings/webm/{muxing_id}/drm/speke/{drm_id}', pathParamMap).then((response) => {
      return map(response, SpekeDrm);
    });
  }

  /**
   * @summary List SPEKE DRM of WebM
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the WebM muxing
  ...
  public list(encodingId: string, muxingId: string, queryParameters?: SpekeDrmListQueryParams | ((q: SpekeDrmListQueryParamsBuilder) => SpekeDrmListQueryParamsBuilder)): Promise<PaginationResponse<SpekeDrm>> {
    ...
    return this.restClient.get<PaginationResponse<SpekeDrm>>('/encoding/encodings/{encoding_id}/muxings/webm/{muxing_id}/drm/speke', pathParamMap, queryParams).then((response) => {
