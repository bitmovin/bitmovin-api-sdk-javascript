...
export default class FairplayApi extends BaseAPI {
  ...
  public create(encodingId: string, muxingId: string, fairPlayDrm?: FairPlayDrm): Promise<FairPlayDrm> {
    ...
    return this.restClient.post<FairPlayDrm>('/encoding/encodings/{encoding_id}/muxings/ts/{muxing_id}/drm/fairplay', pathParamMap, fairPlayDrm).then((response) => {
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/ts/{muxing_id}/drm/fairplay/{drm_id}', pathParamMap).then((response) => {
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<FairPlayDrm> {
    ...
    return this.restClient.get<FairPlayDrm>('/encoding/encodings/{encoding_id}/muxings/ts/{muxing_id}/drm/fairplay/{drm_id}', pathParamMap).then((response) => {
      return map(response, FairPlayDrm);
    });
  }

  /**
   * @summary List FairPlay DRMs of a TS muxing
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the TS muxing.
