...
export default class AesApi extends BaseAPI {
  ...
  public create(encodingId: string, muxingId: string, aesEncryptionDrm?: AesEncryptionDrm): Promise<AesEncryptionDrm> {
    ...
    return this.restClient.post<AesEncryptionDrm>('/encoding/encodings/{encoding_id}/muxings/progressive-ts/{muxing_id}/drm/aes', pathParamMap, aesEncryptionDrm).then((response) => {
      return map(response, AesEncryptionDrm);
    });
  }

  /**
   * @summary Delete AES encryption configuration from Progressive TS muxing
   * @param {string} encodingId Id of the encoding
   * @param {string} muxingId Id of the Progressive TS muxing
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/progressive-ts/{muxing_id}/drm/aes/{drm_id}', pathParamMap).then((response) => {
      return map(response, BitmovinResponse);
    });
  }

  /**
   * @summary AES encryption Details of Progressive TS muxing
   * @param {string} encodingId Id of the encoding
   * @param {string} muxingId Id of the Progressive TS muxing
   * @param {string} drmId Id of the AES encryption configuration
   * @throws {BitmovinError}
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<AesEncryptionDrm> {
    ...
    return this.restClient.get<AesEncryptionDrm>('/encoding/encodings/{encoding_id}/muxings/progressive-ts/{muxing_id}/drm/aes/{drm_id}', pathParamMap).then((response) => {
      return map(response, AesEncryptionDrm);
    });
  }

  /**
   * @summary List AES encryption configurations of Progressive TS muxing
   * @param {string} encodingId Id of the encoding
   * @param {string} muxingId Id of the Progressive TS muxing
  ...
  public list(encodingId: string, muxingId: string, queryParameters?: AesEncryptionDrmListQueryParams | ((q: AesEncryptionDrmListQueryParamsBuilder) => AesEncryptionDrmListQueryParamsBuilder)): Promise<PaginationResponse<AesEncryptionDrm>> {
    ...
    return this.restClient.get<PaginationResponse<AesEncryptionDrm>>('/encoding/encodings/{encoding_id}/muxings/progressive-ts/{muxing_id}/drm/aes', pathParamMap, queryParams).then((response) => {
