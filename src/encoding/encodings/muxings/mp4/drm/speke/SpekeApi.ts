...
export default class SpekeApi extends BaseAPI {
  ...
  /**
   * @summary Create SPEKE DRM key provider to an MP4 muxing
  ...
  public create(encodingId: string, muxingId: string, spekeDrm?: SpekeDrm): Promise<SpekeDrm> {
    ...
    return this.restClient.post<SpekeDrm>('/encoding/encodings/{encoding_id}/muxings/mp4/{muxing_id}/drm/speke', pathParamMap, spekeDrm).then((response) => {
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/mp4/{muxing_id}/drm/speke/{drm_id}', pathParamMap).then((response) => {
      return map(response, BitmovinResponse);
    });
  }

  /**
   * @summary Get SPEKE DRM from an MP4 muxing details
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the MP4 muxing
   * @param {string} drmId Id of the SPEKE DRM.
   * @throws {BitmovinError}
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<SpekeDrm> {
    ...
    return this.restClient.get<SpekeDrm>('/encoding/encodings/{encoding_id}/muxings/mp4/{muxing_id}/drm/speke/{drm_id}', pathParamMap).then((response) => {
      return map(response, SpekeDrm);
    });
  }
  ...
  public list(encodingId: string, muxingId: string, queryParameters?: SpekeDrmListQueryParams | ((q: SpekeDrmListQueryParamsBuilder) => SpekeDrmListQueryParamsBuilder)): Promise<PaginationResponse<SpekeDrm>> {
    ...
    return this.restClient.get<PaginationResponse<SpekeDrm>>('/encoding/encodings/{encoding_id}/muxings/mp4/{muxing_id}/drm/speke', pathParamMap, queryParams).then((response) => {
