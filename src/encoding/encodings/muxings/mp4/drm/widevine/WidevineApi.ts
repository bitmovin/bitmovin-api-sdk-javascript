...
export default class WidevineApi extends BaseAPI {
  ...
  /**
   * @summary Create Widevine DRM to an MP4 muxing
  ...
  public create(encodingId: string, muxingId: string, widevineDrm?: WidevineDrm): Promise<WidevineDrm> {
    ...
    return this.restClient.post<WidevineDrm>('/encoding/encodings/{encoding_id}/muxings/mp4/{muxing_id}/drm/widevine', pathParamMap, widevineDrm).then((response) => {
  ...
  public delete(encodingId: string, muxingId: string, drmId: string): Promise<BitmovinResponse> {
    ...
    return this.restClient.delete<BitmovinResponse>('/encoding/encodings/{encoding_id}/muxings/mp4/{muxing_id}/drm/widevine/{drm_id}', pathParamMap).then((response) => {
      return map(response, BitmovinResponse);
    });
  }

  /**
   * @summary Get Widevine DRM from an MP4 muxing details
   * @param {string} encodingId Id of the encoding.
   * @param {string} muxingId Id of the MP4 muxing.
   * @param {string} drmId Id of the widevine drm.
   * @throws {BitmovinError}
  ...
  public get(encodingId: string, muxingId: string, drmId: string): Promise<WidevineDrm> {
    ...
    return this.restClient.get<WidevineDrm>('/encoding/encodings/{encoding_id}/muxings/mp4/{muxing_id}/drm/widevine/{drm_id}', pathParamMap).then((response) => {
      return map(response, WidevineDrm);
    });
  }

  /**
   * @summary List Widevine DRMs of an MP4 muxing
   * @param {string} encodingId Id of the encoding.
